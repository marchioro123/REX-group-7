...
# Black magic in order to handle differences in namespace names in OpenCV 2.4 and 3.0 and above
...
class Camera(object):
    ...
    def __init__(self, camidx, robottype='arlo', useCaptureThread=False):
        ...
        if robottype == 'arlo':
            ...
            self.intrinsic_matrix = np.asarray([1316.0, 0., self.imageSize[0] / 2.0, 0.,
                   1316.0, self.imageSize[1] / 2.0, 0., 0., 1.], dtype = np.float64)
            ...
            self.distortion_coeffs = np.asarray([0., 0., -2.0546093607192093e-02, 3.5538453075048249e-05, 0.], dtype = np.float64)
    ...
    def draw_aruco_objects(self, img):
        ...
        if not isinstance(self.ids, type(None)):
            ...
            for i in range(self.ids.shape[0]):
                cv2.drawFrameAxes(img, self.intrinsic_matrix, self.distortion_coeffs,
